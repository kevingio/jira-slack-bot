...
const scheduleRunSTPDay = require("../../jobs/scheduleRunSTPDay");
...
exports.getJiraWithEmptyStoryPoint = async (req, res) => {
  const endpoint =
    "search?jql=project = WP AND component in (Communication, Topads) AND Sprint in openSPrints() ORDER BY created DESC&fields=created,assignee,status,summary,fixVersions,customfield_10010,components.name,issuetype,customfield_10014";
  ...
  const filteredTickets = issues.filter(
    (item) => !item.fields.customfield_10014
  );
